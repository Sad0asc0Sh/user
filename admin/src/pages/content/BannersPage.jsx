...
function BannersPage() {
  ...
  const columns = [
    ...
    { title: 'ترتیب', dataIndex: 'sortOrder', key: 'sortOrder' },
  ...
  const onNew = () => {
    ...
    form.resetFields()
    setFileList([])
    form.setFieldsValue({ position: 'main-slider', isActive: true, sortOrder: 0 })
    setOpen(true)
  }
  ...
  const onEdit = (b) => {
    setEditing(b)
    setFileList([])
    form.setFieldsValue({ title: b.title, link: b.link, position: b.position, isActive: b.isActive, imageUrl: b.image?.url, sortOrder: b.sortOrder })
    setOpen(true)
  }
  ...
  const save = async () => {
    try {
      ...
      const payload = { title: values.title, link: values.link, position: values.position, isActive: values.isActive, image, sortOrder: values.sortOrder }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <Space>
          <Select placeholder="موقعیت" allowClear style={{ width: 200 }} onChange={setPosition}>
            <Select.Option value="main-slider">اسلایدر اصلی</Select.Option>
            <Select.Option value="middle-banner">بنرهای میانی</Select.Option>
            <Select.Option value="campaign-banner">بنر کمپین</Select.Option>
          </Select>
      ...
      <Modal open={open} onCancel={() => setOpen(false)} onOk={save} okText={editing ? 'ذخیره' : 'ایجاد'} confirmLoading={saving} title={editing ? 'ویرایش بنر' : 'بنر جدید'} width={700}>
        <Form layout="vertical" form={form}>
          ...
          <Form.Item name="position" label="موقعیت" rules={[{ required: true }]}><Select>
            <Select.Option value="main-slider">اسلایدر اصلی (بالای صفحه)</Select.Option>
            <Select.Option value="middle-banner">بنرهای تبلیغاتی (میانه صفحه)</Select.Option>
            <Select.Option value="campaign-banner">بنر کمپین</Select.Option>
          </Select></Form.Item>
          <Form.Item name="sortOrder" label="ترتیب نمایش"><Input type="number" /></Form.Item>
